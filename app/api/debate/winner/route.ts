...
Score each participant out of 10 overall (weighted average of criteria like Logical Coherence, Evidence Use, Relevance, Counterarguments, Feasibility, Persuasiveness), and provide per-contention mini-scores (e.g., 7/10 for ${room.player1?.name || 'Player 1'} on this battleground) with breakdowns.
...
Provide AI-unique insights: Use semantic analysis to flag patterns like fallacy frequency, evidence density, sentiment shifts, argument evolution, or rhetorical strategies; quantify where possible (e.g., "${room.player1?.name || 'Player 1'} used 3 fallacies vs. ${room.player2?.name || 'Player 2'}'s 1").
...
THERE SHOULD ABSOLUTELY BE NO BIAS VS ${room.player1?.name || 'Player 1'} OR ${room.player2?.name || 'Player 2'}. THEY WILL BE ASSESSED FAIRLY, REGARDLESS WHO THEY ARE. 
...
"outcome": "[Who won this contention and why, with granular reasoning citing exact arguments, interactions, and criteria application; include mini-scores like ${room.player1?.name || 'Player 1'}: 8/10, ${room.player2?.name || 'Player 2'}: 6/10; 100-150 words]",
"miniScores": {
"player1": X,
"player2": X
}
...
"aiInsights": [
"Insight 1: [e.g., Semantic analysis shows ${room.player1?.name || 'Player 1'}'s evidence density at 40% higher; flag specific fallacies with cites]",
"Insight 2: [e.g., Pattern of sentiment shifts from assertive to defensive in Round 2]",
"Insight 3: [Additional unique analysis]"
],
"nextSteps": [
"Specific actionable advice for ${room.player1?.name || 'Player 1'}: [e.g., Strengthen rebuttals by addressing X nuance]",
"Specific actionable advice for ${room.player2?.name || 'Player 2'}: [e.g., Bolster evidence with more verifiable cites]",
